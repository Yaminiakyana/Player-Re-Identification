...
MODEL_PATH = "best.pt"
OUTPUT_DIR = "../soccer/output/tracked_frames"
OUTPUT_VIDEO_PATH = "../soccer/output/tracked_video_reid_final.mp4"
LOG_PATH = "../soccer/output/player_tracking_log.csv"

os.makedirs(OUTPUT_DIR, exist_ok=True)

log_file = open(LOG_PATH, "w", newline="")
log_writer = csv.writer(log_file)
log_writer.writerow(["frame", "id", "x1", "y1", "x2", "y2", "cx", "cy"])
...
def draw_frame(frame, results, class_names, frame_idx):
    ...
    if results.boxes.id is not None:
        ...
        for *xyxy, track_id, conf, cls_id in boxes:
            ...
            cx = int((x1 + x2) / 2)
            cy = int((y1 + y2) / 2)
            log_writer.writerow([frame_idx, global_id, x1, y1, x2, y2, cx, cy])
